...
All_Tornadoes <- st_crs(All_Tornadoes, crs = "+proj=lcc +lat_1=50 +lat_2=50 +lat_0=50 +lon_0=-107 +x_0=0 +y_0=0 +a=6371200 +b=6371200 +units=m +no_defs")
...
June6centroid <- groupDayCentroids.sfdfT %>%
  filter(ID == "199906061644")
June6tornadoes <- BigDayTornadoes %>% 
  filter(ID == "199906061644")
```
...
CAPE_Group23 <- crop(CAPE, extent(test))
CAPE_extent<- mask(CAPE_Group23, test)
plot(CAPE_extent)
maxCAPE <- which.max(CAPE_extent) #Returns a pixel number
#Test that this max value equals the one in BigDays.sfdfT
test1 <- getValues(CAPE_extent) 
test1<-as.matrix(test1, ncol=1)
```
...
tm_shape(states) +
  tm_borders() +
tm_shape(J6_1, is.master = TRUE, projection = merc) +
  tm_borders(col = "black", lwd = 3)  +
#tm_shape(June6centroid) +
#  tm_symbols(size = 1.25, col = "black", shape = 24) +
tm_shape(CAPE_latlon) + 
...
```{r}
Group1 <- cluster2torns %>%
  filter(elat >= 46)



G1
Group2 <- cluster2torns %>%
  filter(elat < 46)
...
for(i in 1:1){
  print(i)
rb <- brick(paste0("/Volumes/Zoe's Home/NCARNARR/All/", June6$slug2[i], "/", June6$slug[i]))
  CAPE <- raster(rb, layer = 375)
  HLCY <- raster(rb, layer = 323)
  CIN <- raster(rb, layer = 376)
  UGRD500 <- raster(rb, layer = 117) 
